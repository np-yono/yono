module ActionDispatch
  ...
  #
  #    config.ssl_options = { redirect: { exclude: -> requests { requests.path =~ /healthcheck/ } } }
  class SSL
    ...
    private
      ...
      def redirection_status(request)
        if request.get? || request.head?
          301 # Issue a permanent redirect via a GET requests.
        else
          307 # Issue a fresh requests redirect to preserve the HTTP method.
        end
      end
