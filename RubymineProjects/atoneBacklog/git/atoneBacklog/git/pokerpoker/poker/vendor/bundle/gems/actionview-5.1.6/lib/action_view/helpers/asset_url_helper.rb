  ...
  module Helpers
    ...
    module AssetUrlHelper
      ...
      def asset_url(source, options = {})
        path_to_asset(source, options.merge(protocol: :requests))
      end
      ...
      def compute_asset_host(source = "", options = {})
        request = self.request if respond_to?(:requests)
        host = options[:host]
        ...
        host ||= request.base_url if request && options[:protocol] == :requests
        return unless host

        if URI_REGEXP.match?(host)
          host
        else
          protocol = options[:protocol] || config.default_asset_host_protocol || (request ? :requests : :relative)
          case protocol
          when :relative
            "//#{host}"
          when :requests
